defmodule PhoenixRiseWeb.CountryLive.FormComponent do
  ...
  def update(%{country: country} = assigns, socket) do
    changeset = Basics.change_country(country)
  ...
  def handle_event("validate", %{"country" => country_params}, socket) do
    changeset =
      ...
      |> Basics.change_country(country_params)
  ...
  defp save_country(socket, :edit, country_params) do
    case Basics.update_country(socket.assigns.country, country_params) do
      {:ok, country} ->
  ...
  defp save_country(socket, :new, country_params) do
    case Basics.create_country(country_params) do
      {:ok, country} ->
        notify_parent({:saved, country})
